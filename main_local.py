...
if __name__ == '__main__':
    args = parse_args()
    video_dir = '../videos/videodata/20240430/'
    vfs = os.listdir(video_dir)
    # video_urls = [os.path.join(video_dir, x) for x in vfs]
    # video_urls = ['../videos/videodata/20240430/3ZRZVQJYA-M.webm']
    video_urls = ['../videos/videodata/20240430/3b32I2t9Jbs.webm']
    commands = []
    for url in video_urls:
        ...
        for i, (tmp_file, x1, y1, x2, y2) in enumerate(tmp_files):
            ...
            cur_res.extend(res)
            with open(f'results/Clip+{video_name}+P{i}+F{s_frame}-F{e_frame}.txt', 'w') as f:
